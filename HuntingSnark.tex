...
\]
\end{proposition}
The proof of this result is far from trivial, and can be provided on demand by the authors. On the other hand, conjugacy with the Dirichlet distribution, as expressed by the following property, is quite straightforward.
\begin{proposition}
Let $(\boldsymbol{y}_p)_{p\in P}$ be a family of random variables over the simplex of $\mathbb{R}_+^N$, mutually independent given a random variable $\boldsymbol{x}$ over $\mathbb{R}_+^N$. We have
\[
\begin{array}{l}
\begin{array}{ll}
